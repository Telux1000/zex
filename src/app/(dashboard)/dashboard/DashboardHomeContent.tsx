...
export async function DashboardHomeContent({
  ...
  const _homeDataStart = Date.now();
  if (process.env.NODE_ENV === 'development') {
    console.info('[login-perf] dashboard: stats_fetch_start');
  }
  ...
  if (!user) {
    if (process.env.NODE_ENV === 'development') {
      console.info(
        `[login-perf] dashboard: stats_fetch ms=${Date.now() - _homeDataStart} path=no_user`
      );
    }
    return null;
  }
  ...
  if (!business) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.info(
        `[login-perf] dashboard: stats_fetch ms=${Date.now() - _homeDataStart} path=no_business`
      );
    }
    return (
  ...
  if (process.env.NODE_ENV === 'development') {
    console.info(`[login-perf] dashboard: stats_fetch ms=${Date.now() - _homeDataStart}`);
  }
  return (
