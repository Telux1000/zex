...
export async function GET(req: Request) {
  ...
  const businessRes = await admin
    .from('businesses')
    .select(
      'id, name, account_type, owner_id, created_at, admin_suspended_at, admin_deactivated_at, industry_label, industry_other_text'
    )
    .order('created_at', { ascending: false })
  ...
  const accounts = businesses.map((business) => {
    ...
    return {
      id: business.id,
      name: business.name,
      account_type: business.account_type === 'business' ? 'business' : 'individual',
