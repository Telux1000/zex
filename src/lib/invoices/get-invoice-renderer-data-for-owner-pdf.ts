...
/**
 * Load invoice + items for a user who can view the business, suitable for
 * `InvoiceRenderer` (same as dashboard print).
 */
export async function getInvoiceRendererDataForOwnerPdf(
  supabase: SupabaseClient,
  options: { invoiceId: string; ownerUserId: string }
): Promise<{
  data: InvoiceRendererData;
  invoiceNumber: string;
  templateId: InvoiceTemplateId;
}> {
  const { invoiceId, ownerUserId } = options;
  ...
  const templateId = normalizeInvoiceTemplateId((row as { template_id?: string | null }).template_id);
  ...
  return {
    ...
    invoiceNumber: String(row.invoice_number ?? 'document'),
    templateId,
  };
}
